...
export default function SectionWeightHistory({ data, onChange, lang }: Props) {
  const t = (key: keyof typeof section6): string => {
    const val = section6[key]?.[lang] ?? section6[key]?.pl ?? key;
    return Array.isArray(val) ? val.join(' / ') : val;
  };

  const keys = Object.keys(section6).filter(
    (k): k is keyof typeof section6 =>
      typeof k === 'string' && k.startsWith('q6_') && !k.endsWith('_options')
  );

  return (
    <div className="space-y-4 bg-[#f7f7f7] p-4 rounded shadow mt-4">
      <h3 className="text-xl font-semibold border-b pb-2">
        6. {t('section6_title' as keyof typeof section6)}
      </h3>
